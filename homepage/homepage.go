...
func handle(w http.ResponseWriter, req *http.Request, cfg *config.Config, userAccessToken, collectionID, lang string, homepageClient HomepageClienter, rend RenderClient) {
	...
	m := mapper.Homepage(config.Config{}, lang, basePage, homepageContent.MainFigures, homepageContent.FeaturedContent, homepageContent.AroundONS, homepageContent.ServiceMessage, homepageContent.EmergencyBanner)
