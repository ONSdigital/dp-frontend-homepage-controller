...
func Init(ctx context.Context, r *mux.Router, cfg *config.Config, cache cache.List, hcHandler func(http.ResponseWriter, *http.Request), homepageClient homepage.Clienter, renderClient homepage.RenderClient) {
	log.Info(ctx, "adding routes")
	r.StrictSlash(true).Path("/health").HandlerFunc(hcHandler)
	r.StrictSlash(true).Path("/census").Methods("GET").HandlerFunc(census.Handler(cfg, cache, homepageClient, renderClient))
